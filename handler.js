...
module.exports.slotRun = async (event, context) => {
  ...
  const now = await pool.query(`select * from(select PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY rtp) as rtp, COUNT(user_name) as spin, machine_id,user_name,user_id from (select (CAST(total AS FLOAT)/CAST(totalbet AS FLOAT)) as rtp, machine_id, user_name,user_id from (select (totalwinnings+jackpotwins) as total, totalbet, machine_id,user_name,user_id from (select machine_id,user_id ,user_name,( case when win_amount is null then 0 else win_amount end) as JackpotWins,winnings as totalwinnings,( case when was_free_game = true then 0 else bet end) as TotalBet from "dev"."public"."fact_slot_machine_spin" join "dev"."public"."dim_user" as dim_user on user_name=SUBSTRING(REPLACE(spin_id,'.','_'),1,CHARINDEX('-',spin_id)-1) full outer join "dev"."public"."fact_slot_jackpot" as jackpot  on "dev"."public"."fact_slot_machine_spin".spin_id = jackpot.spin_id where "dev"."public"."fact_slot_machine_spin".time >  ${END_DATE.getTime()} AND "dev"."public"."fact_slot_machine_spin".time < ${START_DATE.getTime()} AND stage = 'live' AND in_tournament_mode = false )) where total != 0 AND totalbet != 0  ) group by user_name,machine_id,user_id ORDER BY user_name) where rtp > 2`);

  //TODO - new query that looks at stage preview
  const dev_now = await pool.query(`select * from(select PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY rtp) as rtp, COUNT(user_name) as spin, machine_id,user_name,user_id from (select (CAST(total AS FLOAT)/CAST(totalbet AS FLOAT)) as rtp, machine_id, user_name,user_id from (select (totalwinnings+jackpotwins) as total, totalbet, machine_id,user_name,user_id from (select machine_id,user_id ,user_name,( case when win_amount is null then 0 else win_amount end) as JackpotWins,winnings as totalwinnings,( case when was_free_game = true then 0 else bet end) as TotalBet from "dev"."public"."fact_slot_machine_spin" join "dev"."public"."dim_user" as dim_user on user_name=SUBSTRING(REPLACE(spin_id,'.','_'),1,CHARINDEX('-',spin_id)-1) full outer join "dev"."public"."fact_slot_jackpot" as jackpot  on "dev"."public"."fact_slot_machine_spin".spin_id = jackpot.spin_id where "dev"."public"."fact_slot_machine_spin".time >  ${END_DATE.getTime()} AND "dev"."public"."fact_slot_machine_spin".time < ${START_DATE.getTime()} AND stage = 'preview' AND in_tournament_mode = false )) where total != 0 AND totalbet != 0  ) group by user_name,machine_id,user_id ORDER BY user_name) where rtp > 2`);
